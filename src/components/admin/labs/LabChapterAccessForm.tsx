...
export function LabChapterAccessForm({
  initialData,
  sectionId,
  chapterId,
}: {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      toast.error(
        error.message || "An error occurred while updating the access of lab"
      );
    }
  };
  return (
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                  ...
                  <div className="space-y-1 leading-none">
                    <FormDescription>
                      Check this box if u want to make this chapter free for
                      preview
                    </FormDescription>
                  </div>
                </FormItem>
