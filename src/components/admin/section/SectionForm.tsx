...
export function SectionForm({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await uploadSection(courseId, values);
  ...
  const onEdit = async (_id: string) => {
    router.push(`/admin/courses/${courseId}/sections/${_id}`);
  };
  ...
  return (
    <div className="relative mt-6 border bg-slate-100 rounded-md p-4">
      ...
      {!isCreating && (
        ...
        >
          {!initialData?.sections?.length && "No chapters yet."}
          <SectionsList
            onEdit={onEdit}
            onReorder={onReorder}
