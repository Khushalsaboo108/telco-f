...
import { getPublishedCourseById } from "@/app/actions/course.action";
...
export default async function SingleCourse({ params }: PageProps) {
  ...
  let chapterTotal: number = 0;
  let sectionTotal: number = 0;

  let courseDetail;
  try {
    courseDetail = await getPublishedCourseById(courseId);
  } catch (error) {
    courseDetail = null;
  }
  ...
  function getCourseCounts(courseDetail: any) {
    ...
    const validSections = courseDetail.data.sections.filter(
      (section: any) =>
        section.isPublished &&
        section.chapters.some((chapter: any) => chapter.isPublished)
    );

    // Count all published chapters across all valid sections
    const publishedChaptersCount = validSections.reduce(
      (total: number, section: any) => {
        const sectionPublishedChapters = section.chapters.filter(
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          <div className="lg:col-span-2">
            ...
            <div className=" block lg:hidden ">
              {courseDetail?.data && (
                <CoursesRight courseDetail={courseDetail.data} id={courseId} />
              )}
            </div>
            ...
            <section className="mb-8">
              ...
              <CourseAccordion section={courseDetail?.data?.sections || null} />
            </section>
